 ...
 * ---------------------------------------------------------------------------
 * ⚠️ 已退役（2026-09-14）—— 不要再用本脚本做真机走查
 *
 *   本脚本依赖 `miniprogram-automator` 的 ws 协议，而微信开发者工具 3.17.3 起
 *   **不再服务该协议**（实测连不上）。现行轨是 `scripts/verify_miniapp_devtools.py`
 *   （IDE 自带 `wechatide` 工具链，驱动层 `scripts/wechatide_client.py`），见 DR-0009。
 *
 *   本文件保留作**迁移对照**：新脚本的章节映射表与断言仍在参照它。
 *   **未迁移章节的覆盖缺口**登记在 DR-0009「后续增量」，不假装已覆盖。
 * ---------------------------------------------------------------------------
