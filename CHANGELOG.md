...
- **真机预览部署（手机微信扫码体验本机实例）** —— 把「手机访问本机后端」真正打通，并消除两个必踩的坑：
  - `miniapp/project.config.json` 填入真实 AppID `wx4a57f29bc38ca11d`（原为占位值，生成不了预览二维码）；
  - 新增**预览回退身份** `dev_code`：真机预览必须填真实 AppID，而新 AppID 的 `wx.login` 返回 code
    **每次不同**，后端此刻通常还没有 `WX_APP_SECRET` —— 直接拿 code 当身份会**每次登录都新建空账号**
    （表现为「我的货源 / 我的订单」全空，与功能故障无异）。客户端在 `wx.login` 路径附带该角色对应的
    演示账号 code，后端有真实凭据就用真实 openid、没有才回退；**补齐 AppSecret 后自动失效，两端均无需改代码**；
  - `backend/app/modules/auth/service.py`：`code2session(code, dev_code)` 按「凭据是否就绪」选身份；
    `WECHAT_MOCK=false` 时保留 `seed-*` 种子身份直通（本地校验脚本依赖）。
    **回退仅非生产环境生效**：`APP_ENV=production` 一律拒绝，防止用 `seed-*` 冒充演示账号。

### 修复

- `backend/tests/conftest.py`：新增 autouse 的 `force_wechat_mock`。`config` 的 env_file 会叠加
  `.env.local`，本机一旦写入真实微信凭据，单元测试就会真的去调 `code2session`（网络依赖、超时、不可复现）。
  与 `test_agent.py` 的 `force_llm_mock` 同理做隔离。
- `scripts/verify_ui_interactions.js`：一条断言把源码**写死**为 `login(silent)`，调用签名一变即误报。
  改为只校验 `enterRole` 内部调用顺序（映射账号 → login → bindRole → switchRole），不绑定参数写法。
- **`miniapp/utils/request.js`：域名校验的失败提示原本一律指向「去开发者工具勾选『不校验合法域名』」，
  而那个勾选只对电脑模拟器生效 —— 真机用户照做后依然被拦，弹窗等于把人引向一个永远修不好的开关。**
  现按运行环境分流：真机上直接提示「点右上角 `···` → 开发调试」，只有模拟器才提示改工具设置。
  新增 `isRealDevice()`（与 `resolveBaseUrl` 共用判定口径）；`verify_ui_interactions` 补一条断言
  覆盖真机分支（214 → **215**）。
- **真实 AppID 位数不足会把项目静默降级成「游客模式」** —— 首次填入时误写为 17 位的
  `wx4a57f29bc38ca1d`（正确为 18 位的 `wx4a57f29bc38ca11d`）。微信 AppID 固定为 `wx` + 16 位；
  **位数不足时开发者工具不报「格式错误」**，而是把项目当游客项目加载，于是四个症状并发：
  点「预览」没反应、真机调试灰显、提示「该用户不是小程序的开发者」、右上角显示游客模式 ——
  其中那句提示会把人引向「开发者权限」，实为误导。已在 `scripts/verify_miniapp.js` 增补第 8 项门禁：
  AppID 必须匹配 `^wx[\da-z]{16}$`（占位值 / `touristappid` / 空值放行），
  自测确认旧值被拦截、真值放行，杜绝此类手抄位数错误再次入库。
- **「多端模式」切换留下的入库残留**：开发者工具切到多端应用时会改写
  `miniapp/project.config.json` 的 `projectArchitecture` 为 `multiPlatform`，并生成
  `app.miniapp.json` / `project.miniapp.json` / `i18n/`；**切回小程序模式时它并不会改回来**。
  这些字段与文件在代码中零引用，且多端是另一套运行时（原生 App、不走本项目的局域网后端，
  其自报的「当前网络不稳定」与本项目无关，极易误判成网络故障）。
  已清空该字段并把三个文件移出仓库（备份于 `tmp/multiapp-residue/`），
  `verify_miniapp` 的 JSON 计数由 23 回到正确的 20。

### 变更

- `miniapp/assets/`：`home-hero.jpg` 150.0 → 106.2 KB、`port-map.jpg` 71.0 → 54.1 KB，
  合计 **221.1 → 160.3 KB**，通过微信「代码质量」的「图片和音频资源不超过 200 K」检查
  —— 该规则看的是**合计体积**，两张图单个都没超 200 K 也会报。
  策略为宽度保持 1024、JPEG 质量 76 + progressive + optimize；原件备份在 `tmp/assets_backup/`，
  脚本 `tmp/apply_compress.py` 可重跑。

### 文档

- `docs/真机预览部署指南.md`：新增 5.1「预览回退身份」；把「两种配置必须成对」表中
  「真实 AppID + `WECHAT_MOCK=true`」由 ❌ 改为 ✅（该坑已被回退机制消除）；第四节补
  **私有配置覆盖公共配置**的告警。第六节补两类必踩坑 —— ①**手机端「开发调试」是绕域名校验的
  唯一途径**（工具里那个勾选对真机无效），且**每次重新预览都要复查该开关**；②**手机与电脑必须同网段**
  —— 不同网段（双 SSID、访客网络等）即使能互相 ping 通，微信也会报**完全相同的**
  `url not in domain list`，极易误判成后端故障。故障排查表补「对话页提示『（未获得回答）』
  其实是请求被拦的失败兜底，并非 AI 故障」；新增第十一节说明「代码质量」两项的处置。
- `docs/汇报演示脚本.md`：修正两处与实现不符 —— ①后端地址已改为按运行平台自动解析
  （`DEVTOOLS_BASE_URL` / `LAN_BASE_URL`），不再是一个 `BASE_URL` 常量；②原文建议私有配置写
  `appid: "touristappid"` 并称公共配置是占位符，现状恰好相反：**私有文件的 `appid` 优先于公共配置**，
  写成游客态会让真机预览失效 —— 已改写为说明这一覆盖关系。

- **新增《甲方实操评审指南》（`docs/甲方实操评审指南.md`）** —— 面向甲方评审人员的**独立实操手册**，
  与现有文档分工明确：`README` 是工程速览、`docs/汇报演示脚本.md` 面向主讲人（答辩口径），
  本指南解决"甲方拿到代码后如何**自己**跑通并验收"。
  - 明确**只需要装两样东西**（Python 3.12+ / 微信开发者工具），并显式列出**不需要准备**的
    六项（MySQL、Redis、向量库、大模型 Key、微信 AppID、域名备案/公网服务器）；
  - 五步操作路径：核对版本 → 启动后端 → 铺演示数据 → 小程序逐屏验收 → 自动化自证；
  - 每一步都给出**预期输出与"这一步证明了什么"**（含 `seed_demo.py` 的真实输出样例、
    形如 `{"status":"ok","env":"development"}` 的响应体）；
  - 用 `boundingClientRect`/真接口实测**校正了三处与实现不符的描述**：货主端 CTA 是
    「查看匹配船源」而非「智能找船」、下单需在弹窗填「成交运费」、港口运营台因 C 端入口下线
    需改走 `/docs` 接口复现（附实测 `409` 原始响应体）；
  - **18 项可勾选验收清单** + **10 条"已知边界与不验收项"**（主动交代占位与刻意限制，
    避免把原型占位误判为缺陷）+ 9 条故障排查表 + 技术亮点与可验证证据的对照索引。
- `README.md`：文档索引表补上本指南；顺手删除重复的「汇报演示脚本」行。
