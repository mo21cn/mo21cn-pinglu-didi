...
def test_matrix_size_matches_baseline():
    ...
    * `GET  /contracts/{contract_artifact_id}/signature-evidence` —— 全部版本的清单。
    ...
    92 → 100（**S7-3 结算与收付依据**，§10.1 第 11 步 / 合同 S4 段第 11 条；裁定 Q5）：
    ...
    * **仅客户本人**（`POST /settlements/{id}/customer-confirm`）—— `GUARD_OWNER_SELF`：
    ...
    ⚠️ **与本条并行的 S7-2（另一条 PR）也在同一个计数上 +2** ⇒ 两条都合入后是 **102**；
    本条按**本分支自己的树**断言 100：断言的是"我的树上矩阵有几条"，
    不是"合并之后会有几条"（后者断言不了，也不该断言）。
    """
    assert len(sm.SCOPE_MATRIX) == 100
