...
> ⭐ **后续（2026-09-17）**：第 2 节那个缺陷**已修**（提示词改成"可原样复制的来源目录"，
> 见 `DEMO-1-interface-delta.md` §7.7 第 42 条 / PR #131），并已**经真实会话／作业入口复证**：
> 见同目录 `D1-03-live-session-notes.md` —— `source_kinds=['attachment_text']`、
> `unverified_sources=[]`，**未复现**本文件第 2 节的形态。
> 下面这段"本轮不修"的记录**保留原文**（它解释了当时为什么押后），但**结论已作废**。
