...
| 255 | ⭐ **走查第 53 章（结案）**：`PASS=11 / FAIL=0 / NOT_RUN=1`（截图 `walk-20260918-233709/`，含 `53-结案五维清单.jpg`） | 章节 `53 结案：第 12 步的入口与五维清单`。逐条：① 后端读端点可用（`ready=False` ＋ `missing[]`）｜② 详情页出现「结案」入口且**清单先于按钮**给出｜③ **五格恒出五格**｜④ **两处同结论**（界面列出的缺项 code 与服务端读端点逐条一致）｜⑤ 「结案」是**页内确认条**（不是原生弹层）｜⑥ **点结案被服务端拦，页面把缺项逐条摆出来** —— 读数原文：`还不能结案（缺 4 项）：有 7 个任务尚未处置完成（结案要求全部完成，或按处置规则取消）；有 1 条**阻断类**案件尚未解决…；有 1 条案件未关闭 —— 残留必须**显式处置**（关闭时必填 closure_di…`｜⑦ 被拦之后委托**状态未变**（`claimed`，不可逆动作没有半途生效）｜⑪ 无新增 console 报错。<br>⛔ **1 条 `NOT_RUN`：⑧「齐备 ⇒ 结案成功」** —— 需要一张五维度齐备的委托（任务全处置 ＋ 必需证据齐 ＋ 案件全关闭 ＋ 结算已批准且客户已确认 ＋ 余额结清），现成种子都停在前置不齐那一步 ⇒ 缺一个「五维齐备」夹具，如实记 `NOT_RUN`（⛔ 不用接口伪造一个"已齐备"的读数）。<br>⚠️ **环境事实**：首跑被 IDE 残留实例（21 个）挡住，清理后复跑成功 —— 与 O-8 同一环境故障，不是业务结论。 |
| 256 | ⭐ **「五维齐备」夹具**（`backend/scripts/seed_entrust_completion_ready.py`）—— 第 12 步「齐备 ⇒ 结案成功」的载体 | 铺一张**可直接结案**的委托：受理 → 任务（建 / 开工 / 完成）→ 费用（草稿 → 确认）→ 结算版本 → 内部确认 → **客户确认**（货主本人）→ 收付依据（金额＝应收 ⇒ 余额结清）。全部走**真实业务命令**，⛔ 不手改状态 —— "可以结案"本身就该是这些命令产生的。<br>⭐ **自证**：末尾直接调 `closure_readiness()` 并**断言 `ready is True`**，不齐则**非 0 退出**并逐条打印缺项（fail-closed：⛔ 不留给走查一个"看起来齐备"的库）。<br>幂等：同名委托已存在 ⇒ 复用（不重复铺子对象）。按需夹具 ⇒ 不进 `reset_demo_env.SEED_ORDER`，用 `--extra-seeds` 追加。<br>实测（临时库）：`ready=True` / 缺项 0 / 五维度全 0 / `financial_status=settled`；重跑走复用分支且仍 `ready=True`。 |
| 257 | ⭐ **结案时间接进界面**（合同 §6.4「保留历史」的界面形态） | 后端 `completed_at` 在 #164 就有了，但**前端一个字都没接**（`decorateDetail` 不产出、详情页没有那一行）⇒ 第 12 步的 `inspect retained history` 在界面上**无从查验**。补两处：`decorateDetail` 产出 `completedAt`（未结案为空串，**不编占位时间**）；详情页 `buildFields` **只在真有这个事实时**追加「结案时间」一行。⛔ 未结案不出这一行 —— 「还没有结案」不是「未知」，给它一个「—」会与「时间取不到」长得一模一样。 |
| 258 | ⭐ **第 53 章带「五维齐备」夹具的读数**：`PASS=16 / FAIL=1 / NOT_RUN=0`（截图 `walk-20260918-235640/`） | 命令 `--section 53 --extra-seeds seed_entrust_completion_ready.py`。⭐ **齐备 ⇒ 结案成功拿到设备证据**：⑧-a 界面直接说「可以结案」（缺项为空）｜⑧-b 点结案 ⇒ `status=completed` 且写下**结案时间**｜⑧-c 结案后「结案」入口**消失**。<br>⚠️ **⑧-d FAIL ＝ 断言读错键**（不是产品缺陷）：`fields` 是页面 data 的**顶层**键，首跑写成 `detail.fields` ⇒ 空列表 ⇒ 报成"页面没产出这一行"。已修正，但**未复验** —— 随后两次同配方跑都撞 `ENV_BLOCKED`（IDE 残留实例 15 个、`automation_runtime_info` 连续超时，与 O-8 同一环境故障）⇒ ⛔ 不得写成通过。<br>⭐ 隔离事实：该章**带夹具**时 `PASS=16 / FAIL=1`、**不带夹具**时 `PASS=11 / FAIL=0 / NOT_RUN=1` —— 两条配方都在册，读数互不替代。 |
| 259 | ⭐ **受控重开（S4-c）**：`POST /assignments/{id}/reopen` —— `completed → claimed` | 裁定＝`S4-委托结案状态机口径设计.md` §5.5 **Q2（HO 2026-09-17 已裁）**：只允许有权限、有理由的**纠错重开**；**重开不自动获得取消资格**。<br>· **理由必填**（契约 `min_length=1` ＋ 服务层再兜一次）：没有理由的重开，在审计上等于"结论可以随时改"。<br>· **必须留痕**：新表 `ent_assignment_reopen`（append-only）记下**被撤销的那次结案时间**、理由、操作者、从第几版撤 —— 因为重开会把 `completed_at` 清成 NULL，"曾经结过案"就只剩这张表能回答。<br>· **新权限码** `entrust:assignment:reopen`（独立于 `complete`）：能结案 ≠ 能撤销结案；实现上两者**共用** `_assert_can_operate`（只传不同的码），避免"改一处只改一半"。<br>· 矩阵 104 → **105**；界面加「重开」卡（页内表单 ＋ 页内确认，理由必填）；演示种子补第 9 项权限，齐备夹具的 `REQUIRED_PERMS` 同步。<br>⚠️ 走查 ⑨ 的**设备侧**读数本次未取得（IDE 自动化运行时连续超时，已在切片文档 §6 如实登记）；Q2 的硬证据在 pytest（`test_reopen_never_restores_cancel` 断言"**不该成功**"，不锁死状态码）与 MySQL 并发锚点（`test_reopen_race_exactly_one_winner`，含"输家不多写留痕"）。 |
| 260 | ⭐ **走查闸门失败时给"读数"，不再让人从"页面打不开"倒推真因**（runner 诊断） | 起因是一次**自我更正**：本切片交付当晚第 53 章连续四次没跑成，我前两次把它记成「IDE 残留实例累积」，第三次才发现真因在系统层 —— `OSError: [WinError 1450] 系统资源不足`（`CreateProcess` 直接被拒），**而那一刻 IDE 进程数为 0**。<br>· 新增 `_machine_pressure()`：打印内存占用率／可用物理内存／系统进程数／IDE 进程数 —— 有这三项，"自己的残留"与"整机资源到限"一眼可分（后者可以内存充足但桌面堆配额已满）。<br>· 新增 `_report_spawn_blocked()`：闸门**连子进程都起不来**时不再抛裸 traceback，而是点名「会话级资源到限」＋ 处置（**注销或重启**；⛔ 清 IDE 进程没用）＋ 明写"⛔ 不要靠重跑刷绿灯"。<br>⭐ 同时把就绪闸门失败时的"残留实例"提示升级：点名列 PID 前 8 个并打印**可复制**的清理命令（旧文案带一层多余转义，复制即报错）。 |
| 261 | ⭐ **走查第 54 章：十三步的产物链（读侧断言）** —— 「同一张委托连跑 13 步」的**前提**被首次回答 | `S4-b-结案命令切片.md` §7.1 的**成本最低那条**（路径 ①）。此前 13 步是 13 份**互不相干**的读数：没人知道"第 5 步的候选还在不在""第 7 步的合同是不是还挂着 `assignment_id`"。本章按**步序**串成一张清单：① 受理（授权可唯一定位）｜② 报价成果**逐条查归属**｜③ 计划（航段／任务／前置）｜④ 比价 **≥2 条**可比候选｜⑤ 货量变更历史｜⑥ 费用行｜⑦ 结算版本 ＋ 派生 `settled`｜⑧ 结案齐备（**它是前面每一环的汇总，不是并列的一项**）｜⑨ 整页重取后仍读到同一张单。<br>· 判据三条纪律：**只读**（⛔ 不写库）／**归属是必查项**（产物挂到别的委托上 ＝ 这一步在这张单上没有产物）／缺产物记 `NOT_RUN` 并**点名缺哪一步**（⛔ 不把"没有"读成"做到"）。<br>· ⛔ 章内明写：**本章不声称"13 步在同一张委托上连跑通过"** —— 那是仍没做的那件事（需给走查一个"本章用这张委托"的入口，属**架构改动**）。<br>⚠️ **设备侧读数本次未取得**：与第 53 章同一轮跑，闸门撞 `WinError 1450`（见上条）⇒ `PASS/FAIL/NOT_RUN` **全部为空**，⛔ 不得按"没报错"读成通过。 |
| 262 | ⭐ **"复位状态"把"服务端给的原因"一起吞掉**（e2e 新段抓到的真缺陷，已修） | 给重开 handler 补执行验证（e2e ⑱ 段：空理由⇒不得发请求／有理由⇒恰好一条 POST 且载荷 `expected_revision`＋`reason` 正确／被拒⇒必须显示服务端原因）后立刻报出：`⑱ 被拒但页面没有提示原因 → 409 {"detail":"委托单 3 状态为 claimed…"}`。<br>· 根因：409 分支是「**先设提示、再 `self.load()`**」，而 `load()` 的整页复位块里带着 `completeHint: ''`／`reopenHint: ''` ⇒ 重载回来把刚设的提示**清成空**，用户只看到"点了没反应"。<br>· 修法：复位块**不再复位**这两条"上一次尝试的结果"（它们不是表单状态）；清除点保持在「重新展开表单」（`onCompleteOpen`／`onReopenOpen`）。**同一形态在结案那条也存在，一并修掉**。<br>⭐ 一般化：**"复位状态"与"清掉消息"是两件事** —— 凡是"先设消息、再重载"的分支，复位清单都可能把消息静默吃掉。 |
| 263 | ⭐ **走查第 53 章 ×24 全 PASS ＋ 第 54 章 ×10（9 PASS / 1 NOT_RUN）** —— 重启后补齐三处空缺 | 命令：`--skip-ide --section 53,54 --extra-seeds seed_entrust_canonical.py,seed_entrust_contract_flow.py,seed_entrust_completion_ready.py`；截图 `walk-20260919-090034/`（5 张）。<br>· **⑧-d 复验 PASS**：`字段行=委托编号,归属,提交时间,数据版本,结案时间`（原 FAIL ＝ 断言读错键）。<br>· **⑨-a…⑨-e 全 PASS**：入口出现（`canReopen=True`）｜空理由就地拦（`reopenHint='重开必须给出理由…'`）｜重开成功（`status='claimed'` 且 `completedAt=''`）｜结案入口复现（=1）｜不出现撤回入口（**带正控** `complete-open=1`）。<br>· 第 54 章 ③ 拆成 **③-a**（canonical 航段 ＋ 前置字段是响应里的事实：`legs=3`）＋ **③-b**（任务走 `/entrust/tasks` 找载体：`载体=#6 任务 1 条 归属齐=True`）；⑤ 仍 `NOT_RUN`（货量变更由 ㊿ 章做）。<br>⭐ **修掉两处"载体选错"**：② 的成果载体是 **`seed_entrust_contract_flow.py`**（不是 canonical 种子，后者源码里 `artifact` 出现 0 次）；③-b 的任务载体在 `/entrust/tasks`（`plan.tasks` 只是投影）。两处首跑都因**漏铺夹具/选错端点**报了 FAIL —— 这类必须记 `NOT_RUN` **并点名**，⛔ 不能写成产品缺陷。 |
| 264 | ⭐ **走查通道的第 4 档：等人工在 IDE 窗口里批准授权**（本机环境事实，已写进 runner 与服务端诊断） | 回执长相（新档）：`{"ok": true, "result": {"taskType": "auth", "status": "pending", "message": "Waiting for user authorization."}}`，或 `errorType=CONNECT_ERROR / source=auth / "wait WechatIDE authorization timeout"`。<br>· **判据**：IDE 侧日志 `[mcp-log] session_init … authResult=allow` 与 `[LoginState] storage-miss {key:'cliAuthToken'}` 同时出现 ⇒ 通道本身没坏，缺的是**一次人工点击**。<br>· **必须人工点**：`alreadyTrusted: true` 在新实例上**不作数**（批准是按**实例**的）⇒ 一晚连撞四轮全卡这里，点完立刻通过。<br>· **批准后要复用同一个实例**（`--skip-ide`）：⛔ 别让运行器另起一个（新实例要重新批准）。<br>· ⚠️ 本机的**保活**坑：IDE 是后台任务的子进程 ⇒ 任务结束/超时它会被一起收掉（实测 900s 保活到期后 IDE 消失，下一轮又拿到未批准的新实例）⇒ 跑章节时保活窗口要**盖过整轮**（用 3600s）。<br>⭐ runner 的"残留实例"诊断同时修正：判据从「不是我的 PID」改成「**动手前就存在**且现在仍活着」（`BASELINE_IDE_PIDS`）—— 旧判据会把本轮 IDE 自己的子进程（几十秒后才起来的十几个）冤枉成残留。 |
