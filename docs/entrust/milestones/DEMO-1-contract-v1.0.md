...
## 口径登记（HO 0917-3 及其后续裁定；**合同正文一字未改**）

> 本节同样是**登记元数据**，不是合同条款。合同正文（下方「合同正文起始」之后）仍与
> HO 下发的 v1.0 **逐字一致**；本节只记录"开发方按哪条口径执行、为什么不改正文"。

| # | 事项 | 裁定 | 为什么**不**需要合同修正 |
| --- | --- | --- | --- |
| 1 | §10.1 第 2 步 `A1 uploads the sample quotation` 的**演示口径** | **内置样本＝主演示路径**；**原生文件上传入口保留**（两条通路都在，且复用同一条上传逻辑）；**暂不投入 OS 级自动化** | 合同原文写的就是 `the sample quotation`（**样本**），不是"从系统里挑一个文件" ⇒ 本裁定**不改动任何 mandatory business outcome**，不触发 §8.4 的修正条件。原生文件选择器的自动化属**工具能力边界**（OS 级弹层不在小程序渲染树里），按 §7 语义归入暂停：`"Paused" means outside DEMO-1's additional implementation obligation.`；若它日后成为某条强制验收条件，须按 §7 末段**作为具体依赖上报**，不得静默实现、也不得静默豁免 |
| 2 | S3 的 **A：客户身份与入口** | **已决策**（从"待 HO 裁定"移出）：客户＝当前委托的**货主本人**，复用现有账号；入口扩展现有「我的委托／委托详情」；判权按**具体委托归属**，**组织经理权限不能代替客户确认权限** | §8.2 明令"ordinary implementation details 无需等待额外产品决策"。口径与落地约束登记在 `docs/entrust/S3-发布与客户响应数据设计.md` §1 |
| 3 | S3 的 **B：不可变发布版本** | **已决策**：独立发布记录绑定**精确成果 revision** + **当时的客户白名单快照**；客户响应绑定该发布记录；**新建内部草稿不等于撤销旧报价**；**已接受的旧版本永久保留** | 同上。数据底座已落（迁移 `ent_commitment`：`ent_leg` / `ent_capacity_candidate` / `ent_offer_release` / `ent_offer_response`） |
| 4 | S3 的 **发布前来源核验** | **列为 S3 必须实现的门槛**：未核验提案**不得**作为已发布依据；人工核验须留下**核验对象与记录**，不能只是一个无证据的勾选 | 是 §9 尾注与 §3.2「来源必须可分」的具体化，不新增业务结果 |

⇒ 由此 **"待决策"清单清空**：A/B 两项已从"待裁定"移入"已决策"，S3 的开工**不再以任何
决策为前置**（范围评估见 `docs/entrust/S3-范围与依赖评估.md` §3）。

⚠️ **"已决策"不等于"已实现"**：S3 目前落地的是**四张表与迁移**，命令、端点、界面**均未实现**；
按 §8.2，进度一律按"哪条业务出口可演示"报，不按 PR / 表 / 端点 / 页面计数。
