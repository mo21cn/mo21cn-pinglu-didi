...
| 项 | 状态 | 依据 |
| --- | --- | --- |
| 三段式方案 + 任务编排 | **已成**（2026-09-18） | 三段式：`ent_leg` ＋ 读端点 `GET /assignments/{id}/plan` ＋ **航段命令**（建段 / **改段留版本** / 版本历史，`scope_matrix` 81 → 84）＋ **写侧界面**（详情页「运输计划」卡上的**加一段 / 改这一段 / 版本历史**；页内表单，非弹层）。任务编排：`ent_workflow_task.precondition_task_id`（一个任务至多一个固定前置，自依赖/循环由服务层拒绝）＋ 读模型与详情页**逐行显示前置**。设备侧：读侧 ㊻ 章、写侧 ㊼ 章。依据见 `DEMO-1-interface-delta.md` §7.21 / §7.22 / §7.24 |
...
| 航段（三段式）结构化对象 | **表、读端点、界面、写命令、写侧界面均已成**（2026-09-18） | `ent_leg`（稳定 ID / 运输方式 / 起终点 / `seq`）已在；读模型 `plan.py` ＋ `GET /assignments/{id}/plan`（`scope_matrix` 80 → 81）＋ 委托详情页「运输计划」卡（与**必需任务前置**同屏）；**航段命令**（建段 / **改段留版本** / 版本历史，`scope_matrix` 81 → **84**；口径＝HO 2026-09-17 三条裁定：谁能建段＝任意验收者/测试者 · 不强制公–水–公 · 改段保留版本）＋ **写侧界面**（加一段 / 改这一段 / 版本历史展开，页内表单）。设备侧：读侧 ㊻ 章、写侧 ㊼ 章。见 `DEMO-1-interface-delta.md` §7.21 / §7.22 / §7.24 |
...
1. ~~**客户侧整条闭环完全缺失**：释放不可变版本 → 客户接受/拒绝 → 合同草稿 → 签名，
   无端点、无页面，`project_for_customer` 是**没有调用方的孤儿函数**。~~
   ⇒ **2026-09-17 更新**：发布 / 客户响应 / 客户下载已落（**端点与界面均已上线**，见
   `DEMO-1-interface-delta.md` §7.11），合同派生**后端已落**（§7.12，**界面未做**）。
   仍在缺口里的只有**签署证据的取证动作**（上传/记录并绑到合同版本）；
   ⚠️ 且"界面已上线"≠"设备侧走查已过" —— 合同把「对客确认」列为必经环节
   （D1-07 / D1-08 / D1-11），**本文件不声称任何 D1 通过**。
...
> **进度（2026-09-18 更新）**：第 4/5/6/8 项已落地（后端 + 界面），
> 第 7 项**两侧都已落**：合同派生的后端（`S3-合同派生切片.md` §1–§8）＋
> **签署证据的后端与界面**（同文件 §10；新表 `ent_contract_signature_evidence`、
> 两条端点、详情页「合同核对稿」卡）。第 1/2/3 项**已推进**（逐条取证后更新）：
> **三段式编排**＝`ent_leg` 读/写命令＋任务固定前置（`precondition_task_id`）；
> **比价接 UI**＝#146 已上线两条候选并排比较；**选择≠资源确认门禁**＝后端已实现且有专门用例。
>
> **第 7 项的收尾口径（HO 2026-09-18 裁定）：就地降级为「标注样件即止」。**
> 合同 §10.1 第 7 步的原文是 `Create the contract and record **labeled sample**
> signature evidence`，D1-08 的后半句是 `evidence and status remain **distinct from**
> live e-signature`（**不得等同**，不是"实现电子签"）。本切片落地的正是这条边界
> （`mode` 服务端写死 `labeled_sample`、常驻声明、证据绑版本、同版同形态只一条）
> ⇒ **按判据原文成立**。⛔ 本轮起**不再**把"合同本身的签署"挂在未做清单里：
...
> ⛔ **不声称任何 D1 通过**；"后端已实现"与"按业务结果可演示"之间隔着**界面与设备侧走查**两道，
> 而"已设备运行"与"业务验收"之间还隔着 HO 的裁定。
