...
7. **把 ㉞ 章那 4 条 `NOT_RUN` 逐条降到"有证据"**——按性价比排序，**进度（2026-09-16）**：
   - ③ **五态到达条件：部分闭环**。`denied` 已由**真实跨组织访问**造出（仅乙组织经理直访甲组织
     委托 ⇒ **API 直证 HTTP 404** ⇒ 界面拒绝态）。⚠️ 只到 **404 这一条通道**：`400/403/404`
     都被映射成 `denied`，其中 **403 到达仍未取证**；`expired`（要等登录态自然过期）与
     `error`（要断网 / 5xx）**仍 `NOT_RUN`**。
   - ② **tabBar 凸起「发布货物」的真实点击 + ⑧b 原生弹层确认键**：**仍未做**。两者都需要
     **OS 级鼠标通道**（`SendInput`），一次可补两处。这是本轮**明确未完成**的一项。
...
10. **补 ⑧b / tabBar 的 OS 级鼠标通道**（第 7 条里唯一还没动的那个手段）：`SendInput` 造真实
    点击，一次补上两处 —— ① `tabBar` 凸起「发布货物」的真实点击；② ⑧b 那笔**原生弹层确认键**
    （工具侧因弹层不在渲染树而记 `LIMITATION`，该 `LIMITATION` **保持原样、不得改成通过**：
    OS 通道是**另一条**证据，两者分别记录）。
11. **授权合并本切片 PR**（`feature/DEMO1-S1-org-queue-claim` ⇒ `develop`）：作者不自批（DR-0003），
    需 HO 明确授权后才可 squash。汇报口径：**㉟ 章 28 项 PASS / FAIL=0**，另有 1 `NOT_RUN` +
    2 `LIMITATION` 不计入通过；**不得**写成"出口判据已通过"（判据是四句合取）。
