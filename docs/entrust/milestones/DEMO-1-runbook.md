...
> 走查章节 = **㊹ 章**（`--section 43,44` 同跑）；设备侧结论见本节末尾的**如实标注**。
...
⚠️ **本轮如实标注（2026-09-18 已订正）**：以上五条的设备侧证据**已于本节产出** ——
走查章节是 **㊹ 章**，正式取证必须 `--section 43,44` **同跑**（㊹ 要发布的载体是 ㊸ 建的那张单；
单跑 ㊹ 会以 `NOT_RUN` 明确说清"缺 ㊸ 建的单"，**不自造数据**）。

**㊹ 章的两处前置（缺一条这一章就跑不出真结论）**

| 前置 | 为什么 |
| --- | --- |
| ㊸ 建的那张委托 | ㊹ 的载体是它。单跑 ⇒ `NOT_RUN`（**不是** `FAIL`：依赖缺失 ≠ 行为违反预期）。 |
| 「〇」节：经**界面**组装一份对客报价 | ㊸ 那条主链路产出的是 `quote_parsed`（船东侧报价），**不在客户可见白名单**内（`offers.release_offer` 一律 400，是**设计**）⇒ 第 6 步没有可发布的载体。本章新增「〇」节，**经界面**在详情页组装一份 `customer_quote`，三个输入框真机打字并断言表单真被写入。 |

⚠️ **走查脚本的一处纪律（本轮踩到的）**：`api_get` / `api_post` 直接拼 `API_BASE`
（`…/api/v1`），路径**少一段 `entrust`** 就打到别的路由 ⇒ 404／`None`；而调用方一律是
`(api_get(...) or {}).get("items") or []` 这种**静默兜底**写法 ⇒ 拿到空列表、断言报「0 条」，
读起来像**产品没发布**。本轮这一处手误一次性造出 3 条假 `FAIL` + 一个
`IndexError: list index out of range`（`mine2[0]` 落在空列表上）。
⇒ 复核口径：改完必须再扫一遍"有没有漏前缀的接口调用"（本仓已加进脚本自检）。

✅ **本轮（2026-09-18）的实体结论**：`--section 43,44` ⇒ **`PASS=85 / FAIL=0`**，
`NOT_RUN` 5 条都是**本轮载体不适用**（1 条前置 + 3 条"被拒⇒核验⇒再发布"剧本，因为人工直写的
载体不在来源门槛下），`LIMITATION` 1 条是 ㊸ 的 OS 原生文件选择器。截图目录见走查输出末行。
⛔ 这**不等于** D1-07 通过：设备侧只答"真机上点得动、写进去读得回来"，**业务验收是 HO 的裁定**。

⚠️ **一处真空通过的订正**：「授权下载清单冻结在发布记录上」这一格，此前当载体**没有附件**时
两侧都是空、`[] == []` **恒真** ⇒ 那是"真空通过"。已改为载体无附件时记 `NOT_RUN`
（判据要能失败才算判据）。取证这条需要让载体本身带附件 —— ㊸ 上传的附件挂在**会话**上，不在成果上。
...
| 模式 | 怎么起 | 能证什么 | ⛔ 不能证什么 |
| --- | --- | --- | --- |
| **真模型**（默认） | `--skip-ide --pay` | 端到端连通性、页面/接口行为，**以及运行中实际触发并验证过的业务规则** | **fixture 专属输出**（如 `mock_content()` 里那条"`amount` 非空 ⇒ 追加 `customer_quote`"的确定性追加）—— ⛔ 不得固定要求真模型复现它 |
| **fixture** | 加 **`--llm-mock`**（翻 `LLM_MOCK=true`） | **fixture 覆盖到的链路规则** | **模型质量**，以及任何依赖模型判断的内容 |

⚠️ **订正（HO 2026-09-20）**：本节上一稿把真模型的限制写成"不能证**链路规则**"，**写得过头** ——
真模型运行中**实际触发并验证的业务规则同样是有效证据**（例如 H 轮真模型下"作业读到的是**附件文本**
而非文件名""提案产出并有来源清单""无未核验来源"这些规则都被真机跑出来了）。
准确的边界只有一条：**真模型不能保证复现 fixture 才有的确定性输出**。
...
```bash
# ① 第 6 步来源门槛（D1-07 / O-1b）：fixture 轮 —— 证 fixture 覆盖到的链路规则
...
# ② 主演示第 1–3 步（㊸）：真模型轮 —— 端到端 ＋ 页面行为 ＋ 真模型下的实际规则
...
#   期望：PASS ｜ ㊸ 29 条；RESULT 里唯一非 PASS 允许是工具边界那条 LIMITATION
```
...
### 8.8 主演示的**固定业务前提**与第 2 步的**两条路径**（HO 2026-09-20 裁定）

* **固定业务前提**（⛔ 不得为迎合剧本改规则；变更前必须以业务记录建立，⛔ 不得失败后改库）：
  **同一张新委托**、800 吨、**不允许拆批**、900 吨候选先有效；变更至 950 吨后**原确认失效**；
  继续完成复核、重新确认/接受与 **A2 接管**。
  ⚠️ 「允许拆批」下的 `NOT_RUN` **不能替代 D1-09**；也⛔ 不能换一张 canonical 委托取 `PASS`
  算作本单的连续演示结果。全文见 **`DEMO-1-walkthrough.md` §2**。
* **第 2 步的两条路径分开记**（⛔ 不得混为一条）：
  1. **本轮选定的内置样本上传通路** ⇒ 按**实际结果**判定（上传/提取/AG-02 作业/采纳/更正，
     真模型轮 `PASS ｜ ㊸` 29 条）；
  2. **系统文件选择器** ⇒ 保留**未验证／工具限制**，⛔ 不宣称通过；⛔ **本轮不扩建 OS 自动化**；
     ⛔ 不把它变成新的交付前置；⛔ 不删除原始限制记录。
* **主演示脚本**（角色、逐步操作、预期结果、诚实回退、业务顺序三条要求）⇒
  **`DEMO-1-walkthrough.md`**；验收逐行结果 ⇒ **`DEMO-1-acceptance.md`**。

### 8.9 ⭐ 主链的**固定章节顺序**（同单连跑照抄这条命令）

```bash
# 起干净实例（一次性；之后一律 --skip-ide 复用，⛔ 不要另起实例）
# ⚠️ 保活要**够长**：起跑前核「保活到期 − 现在 ≥ 预计时长×1.5」
#    （事故：7200s 的保活在 11 章长链跑到一半时到期，**连带杀掉 IDE**，
#      后段读数会长出「页面数据键 0 个／未停在身份选择页」这类**环境死**）
# ⭐ 起了就会**认领**（`miniapp-device-artifacts/_ide_keepalive.json`）：
#    期间另起的 `--prepare-ide` 会**拒跑并点名**（见 §8.10）。
python scripts/run_walkthrough_devtools.py --prepare-ide --kill-all-ide --keepalive 10800

# 主链：第 1–13 步落在**同一张新委托**上（顺序不可打乱）
# ⭐ `--min-keepalive-left 3600`：把「保活剩余必须盖过整轮」做成**机械判据**
#    （不是靠人记得核；剩余不足直接拒跑，退出码 2）
...
**为什么必须是这个顺序**（每一步都是下一步的**业务**前提，不只是脚本顺序）：

| 序 | 章 | 在链上留下的东西 | 谁依赖它 |
| --- | --- | --- | --- |
| 1 | `43` | 客户提交 → 受理（`claimed`）→ 样本上传 → AG-02 → 采纳 → 更正；**锚点记进本进程** | 其后全部章节 |
| 2 | `chain4` | 公路—内河—公路三段 ＋ 任务与前置链（**先建再展**） | 第 4 步判据；`52` 的交接任务 |
| 3 | `45` | **900 吨、不允许拆批**候选 ＋ 确认（第 5 步；链式轮次强制该口径） | `50` 与 `chain9` 的"旧运力" |
| 4 | `44` | 经界面**组装**对客报价 → 发布 → **客户接受精确版本** | `49` 的派生起点 |
| 5 | `49` | 从**本单**那次已接受事实派生合同 ＋ 记签署证据 | 第 7 步判据 |
| 6 | `50` | 案件 → 审批 → 应用 **800→950** ＋ 旧确认复核（不再适用） | `chain9`、`chain12` 的前置 |
| 7 | `chain9` | 复核 ＋ **替换候选重新确认** ＋ **换新成果重新发布 → 客户本人重新接受** | 第 9 步判据 |
| 8 | `55` | A2 **接管**：代次 +1、旧代次提交被拒（同锚同单） | 第 9 步后半 |
| 9 | `52` | 费用 → 缺件补录 → 结算版本 → 内部确认 → **客户确认** → 收付 → 已结清 | `chain11` 的「齐备」前提 |
| 10 | `chain11` | **任务逐条处置**（复核项随之 resolve）＋ **关变更案件**（处置与证据必填） | `53` 的「齐备」前提 |
| 11 | `53` | 结案（齐备⇒成功 ＋ 留痕）→ **受控重开**（理由必填） | 第 12 步判据 |
| 12 | `chain12` | **重进后同一批记录** ＋ **D1-15 同一个值**（货量 950：变更历史／详情页／结算费用行／重进后逐字相同） | 第 13 步判据 |

* ⛔ **不要**把 `chain9`／`chain11`／`chain12` 单独跑或提前跑：它们要求本单已完成前序步骤
  （`chain9`/`chain12` 要 **950 吨**；`chain11` 要**案件与任务已存在**），否则整章 `NOT_RUN`
  并点名缺什么 —— 那是**正确行为**，不是缺陷。
* ⚠️ `44` 有副作用：发布过一次后，同版本不再给发布按钮（设计如此）⇒ 重跑该章会 `NOT_RUN`。
  要重新取证就重跑 `43`（新单新版本）或整体重来一轮。
* ⚠️ 两个 runner **不能并发**（都监听 8000）；runner 现在会**拒跑并点名**持有者。
* ⚠️ **同一实例连跑第 4 轮起**早期章节也开始退化（实测：处理器不执行、页面取不到数）
  ⇒ 要干净读数就**换实例**，⛔ 不靠重跑碰运气。

### 8.10 ⭐ 保活持有者的**唯一性**与**剩余额度**（两条都在 runner 里，⛔ 不靠人记）

IDE 是保活进程的子进程 ⇒ **谁在保活，谁就持有那批实例**。2026-09-20 一天之内因此
**整轮作废两次**（轮 Q、轮 W）：两个保活附着同一批实例，任何一个到期都把 IDE 一起带走，
另一轮的后段读数变成「页面数据键 0 个／路径为空／处理器不执行」—— 而那种轮的 `RESULT`
只是普通的 `FAIL`，**从读数上看不出被污染**。

⇒ 两件事做成了机械判据，**都没有"忽略"开关**：

| 闸门 | 触发条件 | 行为 |
| --- | --- | --- |
| **唯一性** | `--prepare-ide` 时已有一个**活着的**持有者（pid 不是自己） | **拒跑**（退出码 2）并点名 pid ＋ 剩余秒数；⛔ 不起新实例 |
| **剩余额度** | 起跑时 `--min-keepalive-left N` 且持有者剩余 `< N` | **拒跑**（退出码 2）并给出处置；⛔ 不开始跑 |

* 认领记录：`miniapp-device-artifacts/_ide_keepalive.json`（`pid` / `expires_at` / `section`）。
  进程已死的**陈旧记录会被自动清除**；`--prepare-ide` 失败（闸门没过）**不会**留下幽灵认领。
* 日常用法：起实例那条给 `--keepalive`，跑链那条给 `--min-keepalive-left`（建议 ≥ 整轮×1.5）。
* **注入验证**（2026-09-20，证据 `docs/entrust/evidence/2026-09-20-keepalive-gate/`）：
  已有持有者时第二个 `--prepare-ide` ⇒ `rc=2`、点名 `pid=10940 还剩 10686s`、
  IDE 实例数 **21 → 21（一个没多起）**；`--min-keepalive-left 99999` ⇒ `rc=2` 拒跑。
...
### 8.6 主演示第 8 步：委托货量变更 —— **㊿ 章（经界面审批 + 应用）**

> 合同 §10.1 第 8 步：*`Apply the 800→950 change.`*
> 判据 D1-09：*`the 900-tonne candidate unsuitable`*。
> 要求的形态是**同一张委托上**看到
> `800 → 审批 → 950 → 原运力（900 吨候选）不适用 → 复核与重新确认`；
> ⛔ **预置另一张 950 吨的委托不算数** —— 那只能说明系统里有 950。

```bash
python backend/scripts/seed_entrust_canonical.py     # canonical 委托（含 900 吨候选与 800 吨货量）
python scripts/run_walkthrough_devtools.py --section 50
```
...
* ⚠️ **身份的诚实边界**：只有 `seed-owner`（经理）。货量变更的**读写两侧都只有经理侧** ——
  ...
  **不给货主本人放行**（货量本身客户在委托详情里看得到）。
* ⚠️ **写侧的前置由接口铺**（登记变更请求 + 登记受影响项＝本委托 + 运力确认）：
  它们是**前置**、不是本章要取的证。本章要证的是「**经界面**审批与应用」那两步 ——
  「登记案件」本身属 ⑯ 章与 `case-create` 页的取景范围。
* ⚠️ **新值必须超过 900 吨**：脚本取 `max(当前 + 100, 950)`。若取 `当前 + 100 = 900`，
  `capacity` 仍通过（900 ≥ 900 装得下），⑥ 这条判据就白跑一轮（本轮实测踩到）。
* ⚠️ **重跑**：本章会改库（货量 → 950）。重跑时 ⑤c 的**精确那对值**记 `NOT_RUN`
  （脚本自己说明"要复现 `800 → 950` 请重建库"），其余断言按"当前值 → max(当前+100, 950)"照跑。
* ⚠️ **重建库要用 `mv` 而不是 `rm`**：本机沙箱的 safe-delete 守卫会**拦下** `rm -f *.db`，
  而 `migrate.py` 会在旧库上继续跑 ⇒ 种子叠加、案件号翻倍 ⇒ e2e/走查出现
  "上一轮留下的案件把这一轮撞 500"这类**看起来像产品缺陷的假红**（本轮实测踩到）。
* ⚠️ **档位**：本章答的是"真机上点得动 / 写进去读得回来"，**不是业务验收**。
  ⇒ §10.1 第 8 步**不得由我方声称 PASS**（设备证据 ≠ HO 的裁定）。
