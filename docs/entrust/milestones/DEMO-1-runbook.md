...
python scripts/run_walkthrough_devtools.py --prepare-ide --keepalive 10800

# 主链：第 1–13 步落在**同一张新委托**上（顺序不可打乱）
python scripts/run_walkthrough_devtools.py --skip-ide --pay --chain \
  --section 43,chain4,45,44,49,50,chain9,55,52,53,chain12 \
...
| 9 | `52` | 费用 → 缺件补录 → 结算版本 → 内部确认 → **客户确认** → 收付 → 已结清 | `53` 的「齐备」前提 |
| 10 | `53` | 结案（齐备⇒成功 ＋ 留痕）→ **受控重开**（理由必填） | 第 12 步判据 |
| 11 | `chain12` | **重进后同一批记录** ＋ **D1-15 同一个值**（货量 950：变更历史／详情页／结算费用行／重进后逐字相同） | 第 13 步判据 |

* ⛔ **不要**把 `chain9`／`chain12` 单独跑或提前跑：它们要求本单需求已是 **950 吨**，
  否则整章 `NOT_RUN` 并点名「第 8 步未在本单完成」——那是**正确行为**，不是缺陷。
