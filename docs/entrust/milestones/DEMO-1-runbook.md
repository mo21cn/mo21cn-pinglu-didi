...
python scripts/run_walkthrough_devtools.py --prepare-ide --keepalive 7200

# 主链：第 1–9 步落在**同一张新委托**上（顺序不可打乱）
python scripts/run_walkthrough_devtools.py --skip-ide --pay --chain \
  --section 43,chain4,45,44,49,50,chain9 \
...
| 2 | `chain4` | 公路—内河—公路三段 ＋ 任务与前置链（**先建再展**） | 第 4 步判据 |
...
| 6 | `50` | 案件 → 审批 → 应用 **800→950** ＋ 旧确认复核（不再适用） | `chain9` 的前置 |
| 7 | `chain9` | 复核 ＋ **替换候选重新确认** ＋ **追加新版本→重新发布→客户本人重新接受** | 第 9 步判据（A2 接管在 `55`） |

* ⛔ **不要**把 `chain9` 单独跑或提前跑：它要求本单需求已是 **950 吨**，否则整章 `NOT_RUN`
  并点名「第 8 步未在本单完成」——那是**正确行为**，不是缺陷。
