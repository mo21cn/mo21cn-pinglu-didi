...
- **镜像表必须带契约检查**（HO 2026-09-15 定为开发规范，**直接执行**，无需逐次审批）：前端一旦
  **再现**后端的枚举 / 状态机 / 取值域 / 字段映射，就必须有 CI 逐格比对。两步走：先做
  「**常量名 → 值**」映射，再按块切分比对 —— 直接把两侧常量名拿来比会**两边都空**、被判成
  相等（**假绿**）。⚠️ 新增镜像表**若忘了登记就是静默失效**，所以登记与表本身必须同一提交。
- 新增页面须注册路由，且通过 `node scripts/verify_miniapp.js` 路由可达检查。

### 3.6 提交、分支与 PR

- 分支：**`feature-ENT-<编号>-<描述>`**（HO 2026-09-17「0917 决策」批准），从最新 `develop` 创建，PR 合回 `develop`。**不建长期不合并的平行分支**，每个分支完成一个可验收增量、尽早集成。
  - ⭐ **为什么平铺（用 `-` 不用 `/`）**：本机实测过「删一个 ref 连带删掉同目录其它 ref」（删 `feature/X` ⇒ `feature/Y`、`feature/Z` 一起消失），带斜杠的分支名会形成 `refs/heads/feature/` 子目录，正好落在这个故障面上。平铺名从根上规避。
  - ⭐ **仍要保留类型与工作项编号**（`feature` + `ENT-<编号>`），可追溯性不因平铺而丢失。
  - ⚠️ **CI 触发范围（必须同步核对，别凭印象）**：`ci.yml` 的 `push.branches` 是 `[develop, main, "feature/**", "release/**", "hotfix/**"]`，而 `pull_request.branches` 是 `[develop, main]`。平铺名**不匹配任何 push 模式** ⇒ 只有开 PR 后的那一轮触发 ⇒ `check-runs` 是 **6 项**；`feature/**` 命中时会 push + PR 两轮 ⇒ **12 项**。
    **两轮的 6 个 job 完全相同** ⇒ **6 或 12 不是质量标准**；判据是「**必需检查那 6 个 job 名是否全绿**」。
    若确实想在 PR 创建前就拿到推送验证，同一个规范 PR 里把平铺名补进 `push.branches` 即可 —— 但**这不是强制项**，不要为凑 12 项去改 CI。
- **堆叠 PR 必须在开分支时记录「基点 sha」**：一个新 PR 建立在**尚未合并**的旧 PR 上时，把「本分支是从哪个提交开出来的」的全 sha **当场**写进 PR 正文与工作日志。原因是父 PR 被 **squash** 合并后，`update-branch` 会返回 `422`（父分支历史在 develop 上消失），此时要靠 `-s ours` 或纯 API 造合并提交来同步，而**这两条路线都要求"基点 sha 是本分支的祖先"**；父 PR 同步后的新 head **不在本分支历史里**，拿它当基点会当场失败（2026-09-16 实测）。同理，断言"同步没丢内容"时，**期望值取「基点→本分支」的增量，不要取 PR 自报的 `changed_files`** —— 后者在 base 改到 develop 后会被灌大（实测 25 vs 真实 13）。
